...
export default function RegisterPage() {
    ...
    recorderRef.current = createRecorder(async (blob) => {
      ...
      try {
        ...
        if (text.length > 2) {
          setFullName(text)
          setRegState('idle')
          speak('धन्यवाद। अब फेरि बटन थिच्नुहोस् र आफ्नो दश अंकको फोन नम्बर भन्नुहोस्।')
        } else {
          throw new Error()
        }
      } catch {
        setError('नाम बुझिएन।')
        setRegState('idle')
        speak('नाम बुझिएन। फेरि भन्नुहोस्।')
      }
    })
    ...
    recorderRef.current = createRecorder(async (blob) => {
      ...
      try {
        ...
        if (extracted) {
          setPhone(extracted)
          setRegState('idle')
          speak('अब तपाईंको ६ अंकको गोप्य PIN भन्नुहोस्।')
        } else {
          throw new Error()
        }
      } catch {
        setError('फोन नम्बर बुझिएन।')
        setRegState('idle')
    ...
    recorderRef.current = createRecorder(async (blob) => {
      ...
      try {
        ...
        if (extracted) {
          setPin1(extracted)
          setRegState('idle')
          speak('पुष्टिको लागि त्यही PIN फेरि भन्नुहोस्।')
        } else {
          throw new Error()
        }
      } catch {
    ...
    recorderRef.current = createRecorder(async (blob) => {
      ...
      try {
        ...
        if (extracted) {
          setPin2(extracted)
          if (extracted !== pin1) {
            setError('PIN मेल खाएन।')
            setPin1(''); setPin2('')
            setRegState('idle')
            speak('तपाईंले भनेको PIN मेल खाएन। सुरुदेखि PIN भन्नुहोस्।')
          } else {
  ...
  const register = async (name: string, ph: string, p: string) => {
    ...
    if (r.ok) {
      ...
      speak('खाता सफलतापूर्वक दर्ता भयो।')
    ...
    <div className="min-h-screen bg-teal flex items-center justify-center p-4">
      <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }}
        ...
        <div className="flex flex-col items-center justify-center space-y-8 w-full">
          ...
          <div className="w-full space-y-2 bg-glass-light border border-slate-200/20 rounded-xl p-4">
             <div className="flex justify-between items-center">
                <span className="nepali text-slate-200 text-sm">नाम:</span>
                <span className="text-white font-medium">{fullName || '---'}</span>
             </div>
             <div className="flex justify-between items-center">
                <span className="nepali text-slate-200 text-sm">फोन:</span>
                <span className="text-white font-mono">{phone || '---'}</span>
             </div>
             <div className="flex justify-between items-center">
                <span className="nepali text-slate-200 text-sm">PIN:</span>
                <span className="text-white font-mono">{pin1 ? '••••••' : '---'}</span>
             </div>
          </div>
        </div>
        ...
        <p className="text-center text-sm text-slate-300 mt-8 nepali">
          खाता छ?{' '}
          <a href="/login" className="text-white font-medium hover:underline">प्रवेश गर्नुहोस्</a>
        </p>
      </motion.div>
    </div>
  )
