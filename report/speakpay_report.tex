...
exceeding its quality on many tasks. Dettmers et al.~\cite{dettmers2023qlora} extended this with quantization (QLoRA), further reducing memory requirements. These methods make adaptation feasible on a
...
How much data does domain adaptation actually need? We trained our LoRA configuration on increasing subsets of the training data ($N \in \{50, 100, 150, 200, 300, 403\}$). With only 50 examples, WER already dropped from 129.95\% (zero-shot) to 73.9\%, and Transaction Success Rate (TSR) reached 18.3\%. Performance improved steadily with more data, peaking at $N=300$ (TSR 38.3\%), before slightly degrading at $N=403$ (TSR 33.3\%, WER 43.1\%). This suggests that for highly constrained vocabularies like financial commands, fewer than 300 transcribed utterances are sufficient to achieve most of the available gain from LoRA adaptation on a large pretrained model.
